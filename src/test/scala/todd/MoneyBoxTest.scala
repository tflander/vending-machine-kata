...
class MoneyBoxTest extends FunSpec with ShouldMatchers {
  ...
  describe("coin release tests") {
    ...
    it("releases coins to purchase a product") {
      ...
      moneyBox.coinVault should be(new CoinHold(initialQuarters=1, initialDimes=1, initialNickels=1))    
      change should be(0)
      moneyBox.insertedAmount should be(0)
    }
    ...
    it("releases coins to purchase a product using available coins") {
      ...
      moneyBox.insertedAmount should be(40)
      val change = moneyBox.acceptCoinsAndReturnChangeForProductCosting(40)
      moneyBox.coinVault should be(new CoinHold(initialQuarters=0, initialDimes=3, initialNickels=2))    
      change should be(0)
      moneyBox.insertedAmount should be(0)
    }
  ...
  describe("sundry tests") {
    ...
    it("calculates change required to return to the customer") {
      ...
      val change = moneyBox.acceptCoinsAndReturnChangeForProductCosting(40)
      moneyBox.coinVault should be(new CoinHold(initialQuarters=1, initialDimes=2, initialNickels=0))
      change should be(5)
      moneyBox.insertedAmount should be(0)      
    }
  ...
  describe("coin vault tests") {
    it("get requested amount from the coin vault for making change") {
      val moneyBox = new MoneyBox(new CoinHold(initialQuarters=0, initialDimes=2, initialNickels=2))
      moneyBox.vaultAmount should be(30)
      moneyBox.releaseChange(10) should be (Seq(dime))
      moneyBox.vaultAmount should be(20)
      moneyBox.coinVault should be(new CoinHold(initialQuarters=0, initialDimes=1, initialNickels=2))
    }
  }
}